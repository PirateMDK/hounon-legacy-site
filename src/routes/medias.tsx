...
import { ChevronLeft, ChevronRight, X, Film, Image as ImageIcon, Grid3X3, Search, Play } from "lucide-react";
...
type VideoRow = { id: string; title: string; description: string | null; youtube_url: string; thumbnail_url: string | null; video_date: string | null };
type PhotoItem = { src: string; title: string | null; description: string | null; date: string | null };
...
function MediasPage() {
  ...
  const allPhotosBase: PhotoItem[] = (photosRaw && photosRaw.length > 0)
    ? photosRaw.map((p) => ({ src: p.image_url, title: p.title as string | null, description: p.description as string | null, date: (p.photo_date as string | null) ?? null }))
  ...
  const [sort, setSort] = useState<"title-asc" | "title-desc" | "date-desc" | "date-asc">("title-asc");

  const handleFilterChange = (f: "all" | "videos" | "photos") => { setFilter(f); setPage(1); };

  const norm = (s: string | null | undefined) => (s ?? "").toLowerCase();

  const filteredPhotos = useMemo(() => {
    const q = search.trim().toLowerCase();
    let list = allPhotosBase.filter((p) => !q || norm(p.title).includes(q) || norm(p.description).includes(q));
    list = [...list].sort((a, b) => {
  ...
  const filteredVideos = useMemo(() => {
    const q = search.trim().toLowerCase();
    let list = videos.filter((v) => !q || norm(v.title).includes(q) || norm(v.description).includes(q));
    list = [...list].sort((a, b) => {
      if (sort.startsWith("title")) {
        const r = norm(a.title).localeCompare(norm(b.title));
        return sort === "title-asc" ? r : -r;
      }
      const da = a.video_date ?? "", db = b.video_date ?? "";
      const r = da.localeCompare(db);
      return sort === "date-asc" ? r : -r;
    });
    return list;
  ...
  const prev = useCallback(() => setLightbox((i) => i === null ? null : (i - 1 + filteredPhotos.length) % filteredPhotos.length), [filteredPhotos.length]);
  const next = useCallback(() => setLightbox((i) => i === null ? null : (i + 1) % filteredPhotos.length), [filteredPhotos.length]);
  ...
  useEffect(() => {
    ...
    const h = (e: KeyboardEvent) => {
      if (e.key === "Escape") { close(); closeVideo(); }
    ...
    <Page>
      <section className="px-6 py-20 mx-auto max-w-6xl">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mt-10 mb-6">
          {[
            ...
            return (
              <button key={btn.key} type="button" onClick={() => handleFilterChange(btn.key)}
                className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-sm border font-display text-sm tracking-wide transition cursor-pointer ${
                  active ? "bg-gold text-background border-gold shadow-[0_0_20px_oklch(0.78_0.13_80/0.25)]"
        ...
        {filter !== "photos" && (
          <>
            <h2 className="font-display text-2xl text-gold mt-4 mb-6">Vidéos & Conférences {filteredVideos.length > 0 && <span className="text-sand text-sm">({filteredVideos.length})</span>}</h2>
            {filteredVideos.length > 0 ? (
              <div className="grid md:grid-cols-3 gap-6">
                {filteredVideos.map((v) => (
                  <button key={v.id} type="button" onClick={() => setVideoModal(v)}
                    ...
                    <div className="aspect-video bg-black rounded-sm overflow-hidden mb-3 relative">
                      {v.thumbnail_url ? (
                        <img src={v.thumbnail_url} alt={v.title} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 group-hover:scale-105 transition duration-500" />
                      ) : (
                    ...
                    {v.description && <p className="text-sand text-sm mt-1 line-clamp-2">{v.description}</p>}
                  </button>
                ))}
              </div>
            ) : (
              <p className="text-sand italic">Aucune vidéo {search && "ne correspond à votre recherche"}.</p>
            )}
          </>
        )}
        ...
        {filter !== "videos" && (
          <>
            <div className="flex items-baseline justify-between mb-6">
              ...
              <p className="text-sand text-sm">{filteredPhotos.length} photo{filteredPhotos.length > 1 ? "s" : ""} — page {page}/{totalPages}</p>
            </div>

            {filteredPhotos.length === 0 ? (
              <p className="text-sand italic">Aucune photo {search && "ne correspond à votre recherche"}.</p>
            ) : (
      ...
      {lightbox !== null && filteredPhotos[lightbox] && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4" onClick={close} role="dialog" aria-modal="true">
          <button onClick={close} className="absolute top-4 right-4 text-ivory hover:text-gold p-2" aria-label="Fermer"><X size={28} /></button>
          <button onClick={(e) => { e.stopPropagation(); prev(); }} className="absolute left-4 text-ivory hover:text-gold p-2" aria-label="Précédent"><ChevronLeft size={36} /></button>
          <button onClick={(e) => { e.stopPropagation(); next(); }} className="absolute right-4 text-ivory hover:text-gold p-2" aria-label="Suivant"><ChevronRight size={36} /></button>
          <figure className="max-w-5xl max-h-[85vh] flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
            <img src={filteredPhotos[lightbox].src} alt={filteredPhotos[lightbox].title ?? ""} className="max-h-[80vh] w-auto object-contain border-2 border-gold/40" />
            {(filteredPhotos[lightbox].title || filteredPhotos[lightbox].description) && (
              <figcaption className="text-center mt-4 max-w-2xl">
                {filteredPhotos[lightbox].title && <p className="font-display text-gold text-lg">{filteredPhotos[lightbox].title}</p>}
                {filteredPhotos[lightbox].description && <p className="font-italic-serif text-sand text-sm mt-1">{filteredPhotos[lightbox].description}</p>}
              </figcaption>
            )}
            <p className="text-sand text-xs mt-2">{lightbox + 1} / {filteredPhotos.length}</p>
          </figure>
        </div>
      )}

      {videoModal && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4" onClick={closeVideo} role="dialog" aria-modal="true">
          <button onClick={closeVideo} className="absolute top-4 right-4 text-ivory hover:text-gold p-2" aria-label="Fermer"><X size={28} /></button>
          <div className="w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="aspect-video bg-black border-2 border-gold/40 rounded-sm overflow-hidden">
              {isYouTube(videoModal.youtube_url) ? (
                <iframe src={videoModal.youtube_url} className="w-full h-full" allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen title={videoModal.title} />
              ) : (
            ...
            <div className="text-center mt-4">
              ...
              {videoModal.description && <p className="font-italic-serif text-sand text-sm mt-1 max-w-2xl mx-auto">{videoModal.description}</p>}
            </div>
          </div>
        </div>
      )}
