    ...
    <div>
      ...
      <div className="flex gap-2 border-b border-border mb-6">
        <button
          ...
          className={`px-4 py-2 text-sm font-display ${tab === "formations" ? "text-gold border-b-2 border-gold" : "text-sand"}`}
        >
          Catalogue
        </button>
        <button
          onClick={() => setTab("inscriptions")}
          className={`px-4 py-2 text-sm font-display ${tab === "inscriptions" ? "text-gold border-b-2 border-gold" : "text-sand"}`}
        >
          Inscriptions
        </button>
      </div>
    ...
    <div className="space-y-3">
      {(data ?? []).map((i) => (
        <div key={i.id} className="sacred-card">
          <div className="flex justify-between flex-wrap gap-2">
            <div>
              <p className="text-ivory font-medium">
                {i.full_name} — <span className="text-sand">{i.country}</span>
              </p>
              <p className="text-sand text-xs">
                {i.email} · {i.phone} · {new Date(i.created_at).toLocaleString("fr-FR")}
              </p>
          ...
          {i.message && (
            <p className="text-ivory text-sm mt-3 italic border-l-2 border-gold/40 pl-3">
              {i.message}
            </p>
          )}
        </div>
