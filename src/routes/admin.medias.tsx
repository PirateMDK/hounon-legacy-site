  ...
  return (
    <div>
      ...
      <div className="flex gap-2 border-b border-border mb-6">
        <button onClick={() => setTab("photos")} className={`px-4 py-2 text-sm font-display ${tab === "photos" ? "text-gold border-b-2 border-gold" : "text-sand"}`}>Photos</button>
        <button onClick={() => setTab("videos")} className={`px-4 py-2 text-sm font-display ${tab === "videos" ? "text-gold border-b-2 border-gold" : "text-sand"}`}>Vidéos</button>
      </div>
      {tab === "photos" ? (
