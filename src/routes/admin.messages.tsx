...
import { Trash2, Plus, Mail, MessageCircle, Reply, X, History, BellPlus, CheckCircle2, Clock } from "lucide-react";
...
function MessagesAdmin() {
  ...
  const [followupForm, setFollowupForm] = useState<Record<string, { remind_at: string; note: string }>>({});
  ...
  const { data: replies } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("message_replies" as any).select("*").order("created_at", { ascending: false });
      return (data ?? []) as any[];
    },
  });
  ...
  const { data: followups } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("message_followups" as any).select("*").order("remind_at", { ascending: true });
      return (data ?? []) as any[];
    },
  });
  ...
  const logReply = async (m: any, channel: "email" | "whatsapp") => {
    ...
    await supabase.from("message_replies" as any).insert({
      message_id: m.id, channel, body, sent_by: u.user?.id ?? null,
    });
  ...
  const saveFollowup = async (m: any) => {
    ...
    if (!ff?.remind_at) { toast.error("Date requise"); return; }
    const { data: u } = await supabase.auth.getUser();
  ...
  const toggleFollowup = async (fu: any) => {
    await supabase.from("message_followups" as any).update({ done: !fu.done }).eq("id", fu.id);
    qc.invalidateQueries({ queryKey: ["admin-message-followups"] });
  };

  const deleteFollowup = async (id: string) => {
    await supabase.from("message_followups" as any).delete().eq("id", id);
    qc.invalidateQueries({ queryKey: ["admin-message-followups"] });
  };

  // pending follow-ups summary (top banner)
  const now = Date.now();
  const dueFollowups = (followups ?? []).filter((f) => !f.done && new Date(f.remind_at).getTime() <= now);
  ...
  return (
    <div>
      ...
      <p className="text-sand mb-4">Demandes et messages reçus depuis le site. Historique des réponses et rappels de suivi intégrés.</p>

      {dueFollowups.length > 0 && (
        <div className="sacred-card border border-gold/60 mb-6">
          <p className="text-gold font-medium flex items-center gap-2"><Clock size={16} /> {dueFollowups.length} suivi(s) à effectuer maintenant</p>
        </div>
      )}
          ...
          <div key={m.id} className={`sacred-card ${m.status === "nouveau" ? "border-gold/60" : ""}`}>
            <div className="flex justify-between flex-wrap gap-2">
              ...
              <div className="flex gap-2 items-start flex-wrap">
                ...
                {mReplies.length > 0 && (
                  <button onClick={() => setHistoryOpen((s) => ({ ...s, [m.id]: !s[m.id] }))} className="text-xs px-3 py-1 text-gold border border-gold/40 rounded inline-flex items-center gap-1">
                    <History size={12} /> {mReplies.length} réponse{mReplies.length > 1 ? "s" : ""}
                  </button>
                )}
                {pendingFu.length > 0 && (
                ...
                <button onClick={() => setFollowupOpen((s) => ({ ...s, [m.id]: !s[m.id] }))} className="text-xs px-3 py-1 text-gold border border-gold/40 rounded inline-flex items-center gap-1">
                  <BellPlus size={12} /> Planifier suivi
                </button>
                <button onClick={() => setReplyOpen((s) => ({ ...s, [m.id]: !s[m.id] }))} className="text-xs px-3 py-1 text-gold border border-gold/40 rounded inline-flex items-center gap-1">
            ...
            {followupOpen[m.id] && (
              <div className="mt-4 border-t border-gold/20 pt-4 space-y-3">
                <h4 className="text-gold text-sm font-medium flex items-center gap-2"><BellPlus size={14} /> Planifier un suivi</h4>
                <div className="grid md:grid-cols-2 gap-3">
                  <input
                    type="datetime-local"
                    value={followupForm[m.id]?.remind_at || ""}
                    onChange={(e) => setFollowupForm((s) => ({ ...s, [m.id]: { ...(s[m.id] || { note: "" }), remind_at: e.target.value } }))}
                    className="px-3 py-2 bg-input border border-border rounded text-ivory"
                  />
                  <input
                    placeholder="Note (optionnel)"
                    value={followupForm[m.id]?.note || ""}
                    onChange={(e) => setFollowupForm((s) => ({ ...s, [m.id]: { ...(s[m.id] || { remind_at: "" }), note: e.target.value } }))}
                    className="px-3 py-2 bg-input border border-border rounded text-ivory"
                  />
                </div>
                <button onClick={() => saveFollowup(m)} className="btn-primary text-xs">Enregistrer le rappel</button>
              </div>
            )}
            ...
            {mFollowups.length > 0 && (
              <div className="mt-3 space-y-1">
                {mFollowups.map((fu) => {
                  ...
                  return (
                    <div key={fu.id} className={`flex items-center justify-between gap-2 text-xs px-3 py-2 rounded border ${due ? "border-gold/60 bg-gold/10" : "border-border bg-secondary/40"}`}>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => toggleFollowup(fu)} className="text-gold inline-flex items-center gap-1">
                          <CheckCircle2 size={12} /> {fu.done ? "Réouvrir" : "Terminé"}
                        </button>
                        <button onClick={() => deleteFollowup(fu.id)} className="text-destructive"><Trash2 size={12} /></button>
                      </div>
                    </div>
                  );
                })}
            ...
            {historyOpen[m.id] && mReplies.length > 0 && (
              <div className="mt-4 border-t border-gold/20 pt-4 space-y-2">
                <h4 className="text-gold text-sm font-medium flex items-center gap-2"><History size={14} /> Historique des réponses</h4>
                {mReplies.map((r) => (
                  <div key={r.id} className="text-xs border border-border rounded p-3 bg-secondary/30">
                    <div className="flex items-center gap-2 mb-1">
                      {r.channel === "email"
                        ? <Mail size={12} className="text-gold" />
                        : <MessageCircle size={12} className="text-[#25D366]" />}
                      <span className="text-sand">{r.channel === "email" ? "Email" : "WhatsApp"} · {new Date(r.created_at).toLocaleString("fr-FR")}</span>
                    </div>
