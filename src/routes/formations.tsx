...
function FormationsPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("formation_inscriptions").insert({
      ...form,
      formation_id: open.id,
      formation_title: open.title,
    });
    setSending(false);
    if (error) {
    ...
    <Page>
      ...
      <section className="px-6 py-20 mx-auto max-w-6xl">
        ...
        <p className="text-center text-ivory mt-8 max-w-3xl mx-auto font-italic-serif text-lg">
          Hounon Propre transmet son savoir ancestral à ceux qui souhaitent embrasser la voie de la
          tradition vodoun. À l'issue de chaque formation, les apprenants reçoivent un diplôme
          reconnu.
        </p>
        <div className="grid md:grid-cols-3 gap-6 mt-16">
          {(formations ?? []).map((f) => (
            <div key={f.id} className="sacred-card">
              ...
              <p className="text-sand text-xs uppercase tracking-wider mb-4">
                Durée : {f.duration}
              </p>
              <button onClick={() => setOpen({ id: f.id, title: f.title })} className="btn-primary">
                S'inscrire
              </button>
            </div>
          ))}
      ...
      {open && (
        ...
        >
          <form
            ...
            className="sacred-card max-w-lg w-full"
          >
            ...
            <div className="flex gap-3 justify-end">
              <button type="button" onClick={() => setOpen(null)} className="btn-outline">
                Annuler
              </button>
