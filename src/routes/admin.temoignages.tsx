  ...
  return (
    <div>
      ...
      <div className="space-y-3">
            ...
            <div className="flex justify-between flex-wrap gap-2">
              <div>
                ...
                <p className="text-sand text-xs">
                  {t.first_name} · {t.country} · {t.service || "—"}
                </p>
              </div>
              <div className="flex gap-2">
                {tab !== "approved" && (
                  <button
                    ...
                    className="text-xs px-3 py-1 bg-gold/20 text-gold rounded hover:bg-gold/40"
                  >
                    Approuver
                  </button>
                )}
                {tab !== "rejected" && (
                  <button
                    ...
                    className="text-xs px-3 py-1 bg-secondary text-sand rounded hover:bg-destructive/40"
                  >
                    Rejeter
                  </button>
                )}
                <button onClick={() => remove(t.id)} className="text-destructive p-1">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
        ...
        {(data ?? []).length === 0 && (
          <p className="text-sand italic">Aucun témoignage dans cette catégorie.</p>
        )}
      </div>
    </div>
  );
