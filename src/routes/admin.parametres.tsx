...
function SettingsAdmin() {
  return (
    <div className="space-y-8">
      ...
      <SiteContentEditor
        contentKey="logos"
        title="Logos officiels (pied de page)"
        fields={[
          { key: "cncvb_visible", label: "Logo CNCVB-Racine visible", type: "boolean" },
          {
            key: "ministere_visible",
            label: "Logo Ministère de la Culture visible",
            type: "boolean",
          },
  ...
  return (
    <section className="sacred-card">
      <h3 className="font-display text-xl text-gold mb-4 flex items-center gap-2">
        <KeyRound size={20} /> Changer le mot de passe
      </h3>
...
function MfaSection() {
  ...
  const start = async () => {
    const { data, error } = await supabase.auth.mfa.enroll({
      factorType: "totp",
      friendlyName: name || "Authenticator",
    });
    if (error) {
      toast.error(error.message);
      return;
  ...
  return (
    <section className="sacred-card">
      <h3 className="font-display text-xl text-gold mb-2 flex items-center gap-2">
        <Shield size={20} /> Double authentification (2FA / MFA)
      </h3>
      <p className="text-sand text-sm mb-4">
        ...
        1Password…). Un code à 6 chiffres sera demandé à chaque connexion.
      </p>
          ...
          {verified.length > 0 ? (
            <div className="space-y-2 mb-4">
              {verified.map((f) => (
                <div
                  key={f.id}
                  className="flex items-center justify-between bg-secondary/30 px-3 py-2 rounded"
                >
                  <span className="text-ivory text-sm inline-flex items-center gap-2">
                    <ShieldCheck size={16} className="text-gold" />{" "}
                    {f.friendly_name || "Authenticator"}{" "}
                    <span className="text-xs text-sand">({f.factor_type})</span>
                  </span>
                  <button onClick={() => remove(f.id)} className="text-xs text-destructive">
                    Retirer
                  </button>
                </div>
              ))}
            ...
            <div className="flex items-start gap-2 text-amber-400 text-sm mb-4">
              <AlertTriangle size={16} className="mt-0.5" /> 2FA non activée — votre compte est
              protégé uniquement par mot de passe.
            </div>
          )}
          ...
          {!enrolling ? (
            <div className="flex gap-2 flex-wrap items-end">
              <div>
                ...
                <input
                  ...
                  className="px-3 py-2 bg-input border border-border rounded text-ivory text-sm"
                />
              </div>
              <button onClick={start} className="btn-primary text-sm">
                Activer un nouveau facteur 2FA
              </button>
            </div>
          ) : (
            <div className="border border-gold/30 rounded p-4 space-y-3">
              ...
              <p className="text-sand text-xs">
                ...
                <code className="text-gold">{enrolling.secret}</code>
              </p>
              ...
              <input
                ...
                placeholder="000000"
                maxLength={6}
                className="px-3 py-2 bg-input border border-border rounded text-ivory tracking-widest font-mono"
              />
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="text-sm px-3 py-2 text-sand"
                >
                  Annuler
                </button>
              </div>
...
function SessionSection() {
  ...
  return (
    <section className="sacred-card">
      <h3 className="font-display text-xl text-gold mb-2 flex items-center gap-2">
        <LogOut size={20} /> Sessions actives
      </h3>
      <p className="text-sand text-sm mb-4">
        Si vous suspectez un accès non autorisé, déconnectez toutes vos sessions immédiatement.
      </p>
      <button
        onClick={signOutEverywhere}
        className="text-sm px-4 py-2 bg-earth-red/30 border border-earth-red/60 text-ivory rounded hover:bg-earth-red/50"
      >
...
function SecurityScanSection() {
  ...
  const run = async () => {
    ...
    out.push({
      ...
      ok: hasMfa,
      detail: hasMfa ? undefined : "Activez la 2FA ci-dessus.",
    });
    ...
    const { count } = await supabase
      ...
      .eq("role", "super_admin");
    out.push({
      label: "Au moins un Super Administrateur",
      ok: (count ?? 0) >= 1,
      detail: `${count ?? 0} compte(s)`,
    });
    ...
    out.push({
      ...
      ok: true,
      detail: "Pensez à changer le mot de passe par défaut après la première connexion.",
    });
  ...
  return (
    <section className="sacred-card">
      <h3 className="font-display text-xl text-gold mb-2 flex items-center gap-2">
        <ScanSearch size={20} /> Scan de sécurité
      </h3>
      <p className="text-sand text-sm mb-4">
        Vérification rapide de l'état de sécurité de votre compte et du site.
      </p>
      <button onClick={run} disabled={scanning} className="btn-primary text-sm mb-4">
          ...
          {checks.map((c, i) => (
            <li key={i} className="flex items-start gap-2 text-sm">
              ...
              <span className="text-ivory">
                {c.label}
                {c.detail && <span className="text-sand"> — {c.detail}</span>}
              </span>
            </li>
          ))}
        </ul>
