...
function BiographyPage() {
  ...
  const { data: tribute } = useQuery({
    queryKey: ["tribute"],
    queryFn: async () => {
      const { data } = await supabase.from("site_content").select("value").eq("key", "tribute").maybeSingle();
      ...
      <section className="px-6 py-20 mx-auto max-w-5xl">
        <div className="flex flex-col items-center text-center">
          <div className="pulse-ring w-64 h-64 rounded-full overflow-hidden border-2 border-gold/50 mb-8">
            <img src={djimaPortrait} alt="KINWAHO HOUNGUEVI DJIMA — Hounon Propre" className="w-full h-full object-cover" />
          </div>
          ...
          <p className="mt-4 inline-flex items-center gap-2 text-sand">
            ...
            <img src={flagBenin} alt="Drapeau du Bénin" className="h-[18px] rounded-sm" style={{ width: 28 }} />
          </p>
        </div>
          ...
          <>
            ...
            <article className="tribute-card max-w-4xl mx-auto">
              <div className="text-center mb-8">
                <h2 className="font-display text-3xl md:text-4xl gold-text">{(tribute?.title as string) || "Héritage Sacré"}</h2>
                <p className="font-italic-serif text-lg text-sand mt-2">{(tribute?.subtitle as string) || "À KINWAHO HOUNGUEVI ADITI, celui qui a tout transmis"}</p>
              </div>
              <div className="grid md:grid-cols-[auto,1fr] gap-10 items-center">
                <div className="flex flex-col items-center">
                  <div className="father-frame">
                    <img src={fatherPhoto} alt="KINWAHO HOUNGUEVI ADITI" className="w-full h-full object-cover" />
                  </div>
                  <p className="font-display text-gold mt-5 text-lg tracking-wider text-center">{(tribute?.caption_name as string) || "KINWAHO HOUNGUEVI ADITI"}</p>
                  <p className="font-italic-serif text-sand text-center">{(tribute?.caption_subtitle as string) || "Père, Maître & Prédécesseur"}</p>
                </div>
                <div className="font-body text-ivory text-lg leading-relaxed">
                  ...
                  <p className="font-italic-serif text-gold text-center mt-6 text-base">
                    {(tribute?.closing as string) || "— À KINWAHO HOUNGUEVI ADITI, en mémoire et en hommage éternel —"}
                  </p>
                </div>
              </div>
            </article>
        ...
        <div className="sacred-card max-w-3xl mx-auto text-center">
          ...
          <p className="text-ivory mb-6">
            Hounon Propre est reconnu et membre du Conseil National de la Culture Vodoun du Bénin (CNCVB-Racine — Atlantique),
            sous l'égide du Ministère du Tourisme, de la Culture et des Arts de la République du Bénin.
          </p>
          <div className="flex items-center justify-center gap-8 flex-wrap">
            <img src={logoCncvb} alt="CNCVB-Racine" className="h-20 w-auto" />
            <img src={logoMinistere} alt="Ministère du Tourisme, de la Culture et des Arts" className="h-16 w-auto" />
