...
export default function PromoBanner() {
  ...
  return (
    <section className="px-6 my-12">
      <div
        className="mx-auto max-w-6xl relative overflow-hidden rounded-sm border border-gold/60"
      ...
      >
        ...
        <div className="relative z-10 px-8 py-10 md:py-12 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            <h3 className="font-display text-2xl md:text-3xl gold-text leading-tight">
              {data.title}
            </h3>
            {data.subtitle && (
              <p className="font-italic-serif text-ivory/90 mt-2 max-w-2xl">{data.subtitle}</p>
            )}
          </div>
          {data.cta_label &&
            (href.startsWith("http") ? (
              ...
              <Link to={href} className="btn-primary whitespace-nowrap">
                {data.cta_label}
              </Link>
            ))}
        </div>
