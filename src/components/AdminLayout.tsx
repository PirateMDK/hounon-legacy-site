...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center text-sand">Chargement…</div>
    );
  ...
  return (
    <div className="min-h-screen flex bg-background">
      <aside
        className={`${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:sticky lg:top-0 z-40 lg:left-0 inset-y-0 left-0 w-64 bg-card border-r border-border transition-transform flex flex-col h-screen`}
      >
        {/* Header */}
        ...
        {/* Footer */}
        <div className="p-3 border-t border-gold/40 shrink-0 bg-card">
          <p className="text-xs text-sand truncate mb-2" title={email ?? ""}>
            {email}
          </p>
          <button
            onClick={async () => {
      ...
      <div className="flex-1 min-w-0">
        <header className="lg:hidden flex items-center justify-between p-4 border-b border-border bg-card sticky top-0 z-30">
          <button onClick={() => setOpen((o) => !o)} className="text-gold">
            {open ? <X /> : <Menu />}
          </button>
          <span className="font-display text-gold text-sm">ADMIN</span>
          <a href="/" target="_blank" rel="noopener noreferrer" className="text-gold text-xs">
            🌐
          </a>
        </header>
